...
// 1. Define the Props Interface to fix the TS2322 error
interface LiveMatchesSectionProps {
  sportKey?: string;
}

export default function LiveMatchesSection({ sportKey = 'upcoming' }: LiveMatchesSectionProps) {
  // 2. Pass the sportKey to the hook to refetch when league changes
  // const { data: liveMatchesData, isLoading, error, refetch } = useUpcomingMatches(sportKey);
  ...
  const { data: response, isLoading, error } = useLiveMatches(sportKey);
// Update this line to use 'data' (the variable returned by the hook)
  ...
  // --- LOADING & ERROR STATES (Same as your original code) ---
    ...
    <div className="mb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {liveMatches.map((match: any, index: number) => {
          // --- DATA EXTRACTION FOR THE ODDS API ---
          const matchId = match.id;
          const homeTeam = match.home_team;
          const awayTeam = match.away_team;
          
          // Scores are in match.scores: [{name: "Team A", score: "1"}, ...]
          const homeScore = match.scores?.find((s: any) => s.name === homeTeam)?.score || 0;
          const awayScore = match.scores?.find((s: any) => s.name === awayTeam)?.score || 0;
          
          // Odds are in match.bookmakers[0].markets[0].outcomes
          const market = match.bookmakers?.[0]?.markets?.find((m: any) => m.key === 'h2h');
          const hOdds = market?.outcomes?.find((o: any) => o.name === homeTeam)?.price || 0;
          const aOdds = market?.outcomes?.find((o: any) => o.name === awayTeam)?.price || 0;
          const dOdds = market?.outcomes?.find((o: any) => o.name === 'Draw')?.price || 0;
          ...
          return (
            ...
            >
              ...
              <div className="px-4 py-2 bg-slate-800/50 flex justify-between items-center">
                <span className="text-[10px] text-gray-400 uppercase tracking-widest">{match.sport_title}</span>
